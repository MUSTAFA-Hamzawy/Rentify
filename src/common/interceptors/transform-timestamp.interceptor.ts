...
export class TransformTimestampInterceptor<T> implements NestInterceptor<T, T> {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<T> {
    return next.handle().pipe(
      map((data) => this.transformTimestamps(data.data)),
    );
  }
  ...
  private formatItem(item) {
    if (item.created_at) item.created_at = Helpers.timestampDifference(item.created_at);
    if (item.updated_at) item.updated_at = Helpers.timestampDifference(item.updated_at);
    return item;
  }
}